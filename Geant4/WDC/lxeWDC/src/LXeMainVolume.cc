  ...
  , fConstructor(c)
{
 ...
 // G4double housing_X =  scint_x + fD_mtl;
 // G4double housing_Z =  scint_z + fD_mtl;
...
//    new G4Box("housing_box", housing_x / 2., housing_y / 2., housing_z / 2.);
 ...
 fScint_cylinder =
    new G4Tubs("scint_cylinder", 0., fScint_x / 2., fScint_y / 2.,0.*deg, 360.*deg);
  fHousing_cylinder =
    new G4Tubs("housing_cylinder", 0., housing_x / 2., housing_z / 2., 0.*deg,360.*deg);

//////maintank
  mainTankBox = new G4Box("mainTankBox",1*km,1*km,fScint_z+fD_mtl+0.5*m); //charm 

      main_log = new G4LogicalVolume(mainTankBox,G4Material::GetMaterial("Vacuum"),"main_log",0,0,0);
////////////


  fScint_log   = new G4LogicalVolume(fScint_cylinder, G4Material::GetMaterial("LXe"),
                                   "scint_log", 0, 0, 0);
                                
                                  
  fHousing_log = new G4LogicalVolume(
    fHousing_cylinder, G4Material::GetMaterial("Al"), "housing_log", 0, 0, 0);
    

  new G4PVPlacement(0, G4ThreeVector(), main_log, "scintillator",
...
{
 ////////////////////////////////////////////////////
//  scint_x	    = fConstructor->GetScintX();
//  scint_y   	    = fConstructor->GetScintY();
//  scint_z	    = fConstructor->GetScintZ();
